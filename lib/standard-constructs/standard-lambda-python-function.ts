...
export interface StandardPythonFunctionProps
  extends lambdaPython.PythonFunctionProps {
  index?: string;
  handler?: string;
  timeout?: Duration;
...
export class StandardLambdaPythonFunction extends lambdaPython.PythonFunction {
  ...
  constructor(
    scope: Construct,
    id: string,
    props: StandardPythonFunctionProps
  ) {
    ...
    if (!localProps.layers) {
      // Powertools layer
      const powertoolsLambdaLayer =
        lambdaPython.PythonLayerVersion.fromLayerVersionArn(
          scope,
          `${id}-lambdaPowertoolsLambdaLayer`,
          `arn:aws:lambda:${stack.region}:017000801446:layer:AWSLambdaPowertoolsPythonV2-Arm64:12`
        );
    ...
    // }
    if (!localProps.logGroup) {
      localProps.logGroup = new logs.LogGroup(scope, `${id}-LambdaLogGroup`, {
